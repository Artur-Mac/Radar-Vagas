...
class LeverNormalizer(BaseNormalizationAdapter):
    """Deterministic adapter for Lever ATS postings."""

    def source_job_id_from_payload(self, payload: dict[str, Any]) -> str:
        payload_id = super().source_job_id_from_payload(payload)
        company_identifier = payload.get("_company_identifier")
        return f"{company_identifier}_{payload_id}" if company_identifier else payload_id
    ...
    def normalize_payload(
        ...
        if source_job_id is not None and source_job_id != payload_source_job_id:
            raise ValueError(
                f"Lever source job ID mismatch: expected {source_job_id}, "
        ...
        raw_company = payload.get("company_name") or payload.get("_company_identifier")
        ...
        raw_title = payload.get("text") or payload.get("title")
        ...
        raw_url = payload.get("hostedUrl") or payload.get("applyUrl")
        ...
        raw_loc = categories.get("location") if isinstance(categories, dict) else None
        ...
        raw_emp = categories.get("commitment") if isinstance(categories, dict) else None
        ...
        if created_at_raw:
            try:
                if isinstance(created_at_raw, (int, float)):
                    published_at = datetime.fromtimestamp(created_at_raw / 1000.0, tz=UTC)
                else:
                    published_at = datetime.fromisoformat(str(created_at_raw))
            except ValueError:
                pass

        description = cleaned_text or payload.get("descriptionPlain") or payload.get("description")
